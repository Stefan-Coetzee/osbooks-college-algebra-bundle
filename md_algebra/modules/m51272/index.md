...
ⓑ Observe any patterns in the data points. Do you think the association between the variables is positive or negative? Is the pattern linear or nonlinear?
 ...
 (,)
          ...
          ⓔ Find the slope of a line passing through these two points. Interpret its value in terms of the variables being measured.  m=y2-y1x2-x1=
     
    ⓕUse point-slope form or slope intercept form to write the equation of the line passing through these data points. 
 y–y1=m(x–x1) or y=mx+b ⓖ Write this equation in slope-intercept form.  y=mx+b 
...
A professor is attempting to identify trends among final exam scores. His class has a mixture of students, so he wonders if there is any relationship between age and final exam scores. One way for him to analyze the scores is by creating a diagram that relates the age of each student to the exam score received. In this section, we will examine one such diagram known as a scatter plot.
  ...
  where 
   ...
   is the number of chirps in 15 seconds, and 
   
    T(
     c
    )
   
   is the temperature in degrees Fahrenheit. The resulting equation is represented in .
  ...
  Distinguishing Between Linear and Nonlinear ModelsAs we saw above with the cricket-chirp model, some data exhibit strong linear trends, but other data, like the final exam scores plotted by age, are clearly nonlinear. Most calculators and computer software can also provide us with the correlation coefficient, which is a measure of how closely the line fits the data. Many graphing calculators require the user to turn a "diagnostic on" selection to find the correlation coefficient, which mathematicians label as 
   ...
   and the graph of the data,  shows some large data sets with their correlation coefficients. Remember, for all plots, the horizontal axis shows the input and the vertical axis shows the output.
  ...
  The correlation coefficient is a value, 
  ...
  Because the data appear to follow a linear pattern, we can use technology to calculate 
   
    r
   
   Enter the inputs and corresponding outputs and select the Linear Regression. The calculator will also provide you with the correlation coefficient, 
  ...
  We can introduce a new input variable, 
   
    t,
   
   representing years since 1994.The least squares regression equation is:
  ...
  AlgebraicA regression was run to determine whether there is a relationship between hours of TV watched per day 
  ...
  61.966 yearsFor the following exercises, draw a scatter plot for the data provided. Does the data appear to be linearly related?
  ...
  No.For the following data, draw a scatter plot. If we wanted to know when the population would reach 15,000, would the answer involve interpolation or extrapolation? Eyeball the line, and estimate the answer.
  ...
  GraphicalFor the following exercises, match each scatterplot with one of the four specified correlations in  and .
  ...
  TechnologyFor the following exercises, use each set of data to calculate the regression line using a calculator or other technology tool, and determine the correlation coefficient to 3 decimal places of accuracy.
  ...
  ExtensionsGraph 
f(x)=0.5x+10.
   Pick a set of five ordered pairs using inputs 
   x=−2,1,5,6,9
...
Use linear regression to determine a function 
  ...
  Real-World ApplicationsFor the following exercises, consider this scenario: The population of a city increased steadily over a ten-year span. The following ordered pairs shows the population and the year over the ten-year span, (population, year) for specific recorded years:
  ...
  Linear FunctionsDetermine whether the algebraic equation is linear. 
  ...
  3Find the slope of the line graphed.
  ...
  Find the equation of a line with a y- intercept of 
  ...
  More than 250
  Modeling with Linear FunctionsFind the area of a triangle bounded by the y axis, the line 
  ...
  Find the linear function that models the number of people afflicted with the common cold C as a function of the year, 
   
    t.
   
   When will no one be afflicted?For the following exercises, use the graph in  showing the profit, 
  ...
  Find the linear function y, where y depends on 
  ...
  Find and interpret the y-intercept.For the following exercise, consider this scenario: In 2004, a school population was 1,700. By 2012 the population had grown to 2,500.
  ...
  In which subdivision have home values increased at a higher rate?If these trends were to continue, what would be the median home value in Pima Central in 2015?
  $91,625
  Fitting Linear Models to DataDraw a scatter plot for the data in . Then determine whether the data appears to be linearly related.
  ...
  Eight students were asked to estimate their score on a 10-point quiz. Their estimated and actual scores are given in . Plot the points, then sketch a line that fits the data.
  ...
  Determine whether the trend appears to be linear. If so, and assuming the trend continues, find a linear regression model to predict the percent of unemployed in a given year to three decimal places.In what year will the percentage exceed 12%?
   2023
  ...
  2027
  What is the correlation coefficient for this model to three decimal places of accuracy?According to the model, what is the population in 2014?
  7,660
  

      Chapter Practice TestDetermine whether the following algebraic equation can be written as a linear function. 
  ...
  At 6 am, an online company has sold 120 items that day. If the company sells an average of 30 items per hour for the remainder of the day, write an expression to represent the number of items that were sold 
  ...
  Sketch a line with a y-intercept of 
  ...
  The number of people afflicted with the common cold in the winter months dropped steadily by 25 each year since 2002 until 2012. In 2002, 8,040 people were inflicted. Find the linear function that models the number of people afflicted with the common cold 
  ...
  For the following exercises, use the graph in , showing the profit, 
