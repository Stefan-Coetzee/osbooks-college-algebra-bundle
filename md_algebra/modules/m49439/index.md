...
>
>
> ![distance formula graph](../../media/CNX_Coreq_Figure_08_02_001.jpg)
>

<hr/>

1. **Use the Distance Formula**   Use the distance formula to find the distance between the points (−5, −3) and (7,2).
...
| :--- | :--- |
| Label the points (–5, –3) as (*x*1, *y*1) and point (7, 2) as (*x*2, *y*2) and substitute. | $d=\sqrt{{\left(72-\left(-5\right)\right)}^{2}+{\left(2-\left(-3\right)\right)}^{2}}$ |
...
![hyperbola definitional label graph](../../media/hyperboladefgraph.png)

| *Equation* | $\frac{{x}^{2}}{{a}^{2}}-\frac{{\displaystyle {y}^{2}}}{{b}^{2}}=1$ | $\frac{{y}^{2}}{{a}^{2}}-\frac{{\displaystyle {x}^{2}}}{{b}^{2}}=1$ |
| :--- | :--- | :--- |
...
| *Asymptotes* | ( $y=\pm \frac{{\displaystyle b}}{a}x$ | $y=\pm \frac{{\displaystyle a}}{b}x$ |

![hyperbola graph](../../media/hyperbolagraph.png)
...
4. **Graph a hyperbola with center at (0,0).**    Graph  $\frac{{x}^{2}}{25}-\frac{{y}^{2}}{4}=1.$

<details>
<summary>Solution</summary>

![Step 1 is to write the equation in standard form. The the quantity x squared divided by 25 end quantity minus the quantity y squared divided by 4 end quantity is equal to 1 is already in standard form.](../../media/CNX_IntAlg_Figure_11_04_006a_img.jpg)

![Step 2 is to determine whether the transverse axis is horizontal or vertical. Since the x squared term is positive, the transverse axis is horizontal.](../../media/CNX_IntAlg_Figure_11_04_006b_img.jpg)

![Step 3 is to find the vertices. Since a squared is equal to 25, then a is equal to plus or minus 5. The vertices lie on the x-axis and are (negative 5, 0) and (5, 0).](../../media/CNX_IntAlg_Figure_11_04_006c_img.jpg)

![Step 4 is to sketch the rectangle centered at the origin, intersecting one axis at plus or minus a and the other at plus or minus b. Since a is equal to plus or minus 5, the rectangle will intersect the x-axis at the vertices. Since b is equal to plus or minus 2, the rectangle will intersect the y-axis at (0, negative 2) and (0, 2). The rectangle is shown on a coordinate plane with the points (0, 2), (0, negative 2), (negative 5, 0), and (5, 0) labeled.](../../media/CNX_IntAlg_Figure_11_04_006d_img.jpg)

![Step 5 is to sketch the asymptotes, the lines through the diagonals of the rectangle. The asymptotes have the equations y is equal to five-halves times x and y is equal to negative five-halves x. The coordinate plane shows the rectangle with the points (0, 2), (0, negative 2), (negative 5, 0), and (5, 0) labeled and the lines that represent the asymptotes.](../../media/CNX_IntAlg_Figure_11_04_006e_img.jpg)

![Step 6 is to draw the two branches of the hyperbola. Start at each vertex and use the asymptotes as a guide. The coordinate plane shows the rectangle with the points (0, 2), (0, negative 2), (negative 5, 0), and (5, 0) labeled, the lines that represent the asymptotes, y is equal to plus or minus five-halves times x, and the branches that pass through (plus or minus 5, 0) and open left and right.](../../media/CNX_IntAlg_Figure_11_04_006f_img.jpg)
</details>
...
| To write the equation in standard form, divide
 each term by 64 to make the equation equal to 1. | $\frac{4{y}^{2}}{64}-\frac{16{x}^{2}}{64}=\frac{64}{64}$ |
| Simplify. | $\phantom{\rule{1em}{0ex}}\frac{{y}^{2}}{16}-\frac{{x}^{2}}{4}=1$ |
| Since the
*y* 2 -term is positive, the transverse axis is vertical.  Since  ${a}^{2}=16$ then  $a=\text{\xb1}4.$ |  |
| The vertices are on the
*y* -axis,  $\left(0,\text{\u2212}a\right),$  $\left(0,a\right).$  Since  ${b}^{2}=4$ then  $b=\text{\xb1}2.$ | $\left(0,\mathrm{-4}\right),$  $\left(0,4\right)$ |
| Sketch the rectangle intersecting the
*x* -axis at  $\left(\mathrm{-2},0\right),$  $\left(2,0\right)$ and the *y* -axis at the vertices.  Sketch the asymptotes through the diagonals of the rectangle.  Draw the two branches of the hyperbola. | ![.](../../media/CNX_IntAlg_Figure_11_04_007a_img.jpg) |
</details>
...
6. Graph $\frac{{x}^{2}}{9}-\frac{{\displaystyle {y}^{2}}}{16}=1$ .
...
![A shock wave intersecting the ground forms a portion of a conic and results in a sonic boom.](../../media/CNX_Precalc_Figure_10_02_001.jpg)
...
![A hyperbola](../../media/CNX_Precalc_Figure_10_02_002.jpg)
...
![Key features of the hyperbola](../../media/CNX_Precalc_Figure_10_02_003.jpg)
...
![Image](../../media/CNX_Precalc_Figure_10_02_013.jpg)
...
By definition of a hyperbola, ${d}_{2}-{d}_{1}$ is constant for any point $\left(x,y\right)$ on the hyperbola. We know that the difference of these distances is $2a$ for the vertex $(a,0).$ It follows that ${d}_{2}-{d}_{1}=2a$ for any point on the hyperbola. As with the derivation of the equation of an ellipse, we will begin by applying the **distance formula**. The rest of the derivation is algebraic. Compare this derivation with the one from the previous section for ellipses.
 $$
\begin{array}{ll}\phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{d}_{2}-{d}_{1}=\sqrt{{(x-(-c))}^{2}+{(y-0)}^{2}}-\sqrt{{(x-c)}^{2}+{(y-0)}^{2}}=2a  & \text{Distance\ Formula}  \\ \sqrt{{(x+c)}^{2}+{y}^{2}}-\sqrt{{(x-c)}^{2}+{y}^{2}}=2a  & \text{Simplify\ expressions}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}\sqrt{{(x+c)}^{2}+{y}^{2}}=2a+\sqrt{{(x-c)}^{2}+{y}^{2}}  & \text{Move\ radical\ to\ opposite\ side}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{(x+c)}^{2}+{y}^{2}={\left(2a+\sqrt{{(x-c)}^{2}+{y}^{2}}\right)}^{2}  & \text{Square\ both\ sides}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{x}^{2}+2cx+{c}^{2}+{y}^{2}=4{a}^{2}+4a\sqrt{{(x-c)}^{2}+{y}^{2}}+{(x-c)}^{2}+{y}^{2}  & \text{Expand\ the\ squares}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{x}^{2}+2cx+{c}^{2}+{y}^{2}=4{a}^{2}+4a\sqrt{{(x-c)}^{2}+{y}^{2}}+{x}^{2}-2cx+{c}^{2}+{y}^{2}  & \text{Expand\ remaining\ square}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}2cx=4{a}^{2}+4a\sqrt{{(x-c)}^{2}+{y}^{2}}-2cx  & \text{Combine\ like\ terms}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}4cx-4{a}^{2}=4a\sqrt{{(x-c)}^{2}+{y}^{2}}  & \text{Isolate\ the\ radical}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}cx-{a}^{2}=a\sqrt{{(x-c)}^{2}+{y}^{2}}  & \text{Divide\ by\ 4}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{\left(cx-{a}^{2}\right)}^{2}={a}^{2}{\left(\sqrt{{(x-c)}^{2}+{y}^{2}}\right)}^{2}  & \text{Square\ both\ sides}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{c}^{2}{x}^{2}-2{a}^{2}cx+{a}^{4}={a}^{2}\left({x}^{2}-2cx+{c}^{2}+{y}^{2}\right)  & \text{Expand\ the\ squares}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{c}^{2}{x}^{2}-2{a}^{2}cx+{a}^{4}={a}^{2}{x}^{2}-2{a}^{2}cx+{a}^{2}{c}^{2}+{a}^{2}{y}^{2}  & \text{Distribute\}{a}^{2}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{a}^{4}+{c}^{2}{x}^{2}={a}^{2}{x}^{2}+{a}^{2}{c}^{2}+{a}^{2}{y}^{2}  & \text{Combine\ like\ terms}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{c}^{2}{x}^{2}-{a}^{2}{x}^{2}-{a}^{2}{y}^{2}={a}^{2}{c}^{2}-{a}^{4}  & \text{Rearrange\ terms}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{x}^{2}\left({c}^{2}-{a}^{2}\right)-{a}^{2}{y}^{2}={a}^{2}\left({c}^{2}-{a}^{2}\right)  & \text{Factor\ common\ terms}\text{.}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}{x}^{2}{b}^{2}-{a}^{2}{y}^{2}={a}^{2}{b}^{2}  & \text{Set\}{b}^{2}={c}^{2}-{a}^{2}.  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}\frac{{x}^{2}{b}^{2}}{{a}^{2}{b}^{2}}-\frac{{a}^{2}{y}^{2}}{{a}^{2}{b}^{2}}=\frac{{a}^{2}{b}^{2}}{{a}^{2}{b}^{2}}  & \text{Divide\ both\ sides\ by\}{a}^{2}{b}^{2}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \}\frac{{x}^{2}}{{a}^{2}}-\frac{{y}^{2}}{{b}^{2}}=1  &   \end{array}
$$
...
![(a) Horizontal hyperbola with center $\left(0,0\right)$ (b) Vertical hyperbola with center $\left(0,0\right)$](../../media/CNX_Precalc_Figure_10_02_004.jpg)
...
8. **Locating a Hyperbola’s Vertices and Foci**   Identify the vertices and foci of the **hyperbola** with equation $\frac{{y}^{2}}{49}-\frac{{x}^{2}}{32}=1.$
...
10. **Finding the Equation of a Hyperbola Centered at (0,0) Given its Foci and Vertices**   What is the standard form equation of the **hyperbola** that has vertices $\left(\pm 6,0\right)$ and foci $\left(\pm 2\sqrt{10},0\right)?$
...
Solving for ${b}^{2},$ we have
 ...
 $$
\begin{array}{ll}{b}^{2}={c}^{2}-{a}^{2}  &   \\ {b}^{2}=40-36  & \begin{array}{cccc}& & & \end{array}\text{Substitute\ for\}{c}^{2}\phantom{\rule{0.5em}{0ex}}\text{and\}{a}^{2}.  \\ {b}^{2}=4  & \begin{array}{cccc}& & & \end{array}\text{Subtract}.  \end{array}
$$
...
![Image](../../media/CNX_Precalc_Figure_10_02_014.jpg)
</details>

>
> Try It
> 11. What is the standard form equation of the hyperbola that has vertices $\left(\mathrm{0,}\pm 2\right)$ and foci $\left(\mathrm{0,}\pm 2\sqrt{5}\right)?$
>
> <details>
> <summary>Solution</summary>
>
...
![(a) Horizontal hyperbola with center $\left(h,k\right)$ (b) Vertical hyperbola with center $\left(h,k\right)$](../../media/CNX_Precalc_Figure_10_02_005n.jpg)
...
12. **Finding the Equation of a Hyperbola Centered at (*h*, *k*) Given its Foci and Vertices**   What is the standard form equation of the **hyperbola** that has vertices at $(0,\mathrm{-2})$ and $(6,\mathrm{-2})$ and foci at $(\mathrm{-2},\mathrm{-2})$ and $(8,\mathrm{-2})?$
...
Next, we find ${a}^{2}.$ The length of the transverse axis, $2a,$ is bounded by the vertices. So, we can find ${a}^{2}$ by finding the distance between the *x*-coordinates of the vertices.
 ...
 $$
\begin{array}{l}2a=\left|0-6\right|  \\ 2a=6  \\ \phantom{\rule{0.5em}{0ex}}\text\ a=3  \\ {a}^{2}=9  \end{array}
$$
...
Now we need to find ${c}^{2}.$ The coordinates of the foci are $\left(h\pm c,k\right).$ So $\left(h-c,k\right)=\left(-2,\mathrm{-2}\right)$ and $\left(h+c,k\right)=\left(8,\mathrm{-2}\right).$ We can use the *x*-coordinate from either of these points to solve for $c.$ Using the point $\left(8,\mathrm{-2}\right),$ and substituting $h=3,$
 ...
 $$
\begin{array}{l}h+c=8  \\ 3+c=8  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \}c=5  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \}{c}^{2}=25  \end{array}
$$

Next, solve for ${b}^{2}$ using the equation ${b}^{2}={c}^{2}-{a}^{2}:$

 $$
\begin{array}{l}{b}^{2}={c}^{2}-{a}^{2}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \}=25-9  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \}=16  \end{array}
$$
...
>
> Try It
> 13. What is the standard form equation of the hyperbola that has vertices $\left(1,\mathrm{-2}\right)$ and $\left(1,\text{8}\right)$ and foci $\left(1,\mathrm{-10}\right)$ and $\left(1,16\right)?$
>
> <details>
> <summary>Solution</summary>
>
...
14. **Graphing a Hyperbola Centered at (0, 0) Given an Equation in Standard Form**   Graph the **hyperbola** given by the equation $\frac{{y}^{2}}{64}-\frac{{x}^{2}}{36}=1.$ Identify and label the vertices, co-vertices, foci, and asymptotes.
...
![Image](../../media/CNX_Precalc_Figure_10_02_006.jpg)
</details>
...
>
>
> ![Image](../../media/CNX_Precalc_Figure_10_02_007.jpg)
> </details>
>
...
16. **Graphing a Hyperbola Centered at (*h*, *k*) Given an Equation in General Form**   Graph the **hyperbola** given by the equation $9{x}^{2}-4{y}^{2}-36x-40y-388=0.$ Identify and label the center, vertices, co-vertices, foci, and asymptotes.
...
![Image](../../media/CNX_Precalc_Figure_10_02_008.jpg)
</details>

> Try It
> 17. Graph the hyperbola given by the standard form of an equation $\frac{{\left(y+4\right)}^{2}}{100}-\frac{{\left(x-3\right)}^{2}}{64}=1.$ Identify and label the center, vertices, co-vertices, foci, and asymptotes.
>
> <details>
> <summary>Solution</summary>
>
...
>
>
> ![Image](../../media/CNX_Precalc_Figure_10_02_009.jpg)
> </details>
>
>
...
![Cooling towers at the Drax power station in North Yorkshire, United Kingdom (credit: Les Haines, Flickr)](../../media/CNX_Precalc_Figure_10_02_010.jpg)
...
18. **Solving Applied Problems Involving Hyperbolas**   The design layout of a cooling tower is shown in . The tower stands 179.6 meters tall. The diameter of the top is 72 meters. At their closest, the sides of the tower are 60 meters apart.   ![Project design for a natural draft cooling tower](../../media/CNX_Precalc_Figure_10_02_011n.jpg)  Find the equation of the hyperbola that models the sides of the cooling tower. Assume that the center of the **hyperbola**—indicated by the intersection of dashed perpendicular lines in the figure—is the origin of the coordinate plane. Round final values to four decimal places.
...
To solve for ${b}^{2},$ we need to substitute for $x$ and $y$ in our equation using a known point. To do this, we can use the dimensions of the tower to find some point $\left(x,y\right)$ that lies on the hyperbola. We will use the top right corner of the tower to represent that point. Since the *y*-axis bisects the tower, our *x*-value can be represented by the radius of the top, or 36 meters. The *y*-value is represented by the distance from the origin to the top, which is given as 79.6 meters. Therefore,
 ...
 $$
\begin{array}{ll}\frac{{x}^{2}}{{a}^{2}}-\frac{{y}^{2}}{{b}^{2}}=1  & \begin{array}{cccc}& & & \end{array}\text{Standard\ form\ of\ horizontal\ hyperbola}.  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \}{b}^{2}=\frac{{y}^{2}}{\frac{{x}^{2}}{{a}^{2}}-1}  & \begin{array}{cccc}& & & \end{array}\text{Isolate\}{b}^{2}  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \}=\frac{{(79.6)}^{2}}{\frac{{(36)}^{2}}{900}-1}  & \begin{array}{cccc}& & & \end{array}\text{Substitute\ for\}{a}^{2},x,\phantom{\rule{0.5em}{0ex}}\text{and\}y  \\ \phantom{\rule{0.5em}{0ex}}\text{\ \ \ \ \ \ \ \ \ \ \ \}\approx 14400.3636  & \begin{array}{cccc}& & & \end{array}\text{Round\ to\ four\ decimal\ places}  \end{array}
$$
...
>
> Try It
> 19. A design for a cooling tower project is shown in . Find the equation of the hyperbola that models the sides of the cooling tower. Assume that the center of the hyperbola—indicated by the intersection of dashed perpendicular lines in the figure—is the origin of the coordinate plane. Round final values to four decimal places.   ![Image](../../media/CNX_Precalc_Figure_10_02_012n.jpg)
>
> <details>
> <summary>Solution</summary>
>
...
18. $4{x}^{2}-8x-9{y}^{2}-72y+112=0$

19. $-9{x}^{2}-54x+9{y}^{2}-54y+81=0$
...
20. $4{x}^{2}-24x-36{y}^{2}-360y+864=0$

21. $-4{x}^{2}+24x+16{y}^{2}-128y+156=0$
...
22. $-4{x}^{2}+40x+25{y}^{2}-100y+100=0$

23. ${x}^{2}+2x-100{y}^{2}-1000y+2401=0$
...
24. $-9{x}^{2}+72x+16{y}^{2}+16y+4=0$

25. $4{x}^{2}+24x-25{y}^{2}+200y-464=0$
...
![Image](../../media/CNX_Precalc_Figure_10_02_201.jpg)
</details>
...
![Image](../../media/CNX_Precalc_Figure_10_02_203.jpg)
</details>
...
![Image](../../media/CNX_Precalc_Figure_10_02_205.jpg)
</details>
...
![Image](../../media/CNX_Precalc_Figure_10_02_207.jpg)
</details>

38. $-4{x}^{2}-8x+16{y}^{2}-32y-52=0$

39. ${x}^{2}-8x-25{y}^{2}-100y-109=0$

<details>
<summary>Solution</summary>

![Image](../../media/CNX_Precalc_Figure_10_02_209.jpg)
</details>

40. $-{x}^{2}+8x+4{y}^{2}-40y+88=0$

41. $64{x}^{2}+128x-9{y}^{2}-72y-656=0$

<details>
<summary>Solution</summary>

![Image](../../media/CNX_Precalc_Figure_10_02_211.jpg)
</details>

42. $16{x}^{2}+64x-4{y}^{2}-8y-4=0$

43. $-100{x}^{2}+1000x+{y}^{2}-10y-2575=0$

<details>
<summary>Solution</summary>

![Image](../../media/CNX_Precalc_Figure_10_02_213.jpg)
</details>

44. $4{x}^{2}+16x-4{y}^{2}+16y+16=0$

For the following exercises, given information about the graph of the hyperbola, find its equation.
45. Vertices at $\left(3,0\right)$ and $\left(\mathrm{-3},0\right)$ and one focus at $\left(5,0\right).$
...
46. Vertices at $\left(0,6\right)$ and $\left(0,\mathrm{-6}\right)$ and one focus at $\left(0,\mathrm{-8}\right).$
...
48. Center: $\left(0,0\right);$ vertex: $\left(0,\mathrm{-13}\right);$ one focus: $\left(0,\sqrt{313}\right).$
...
51. ![A vertical hyperbola centered at (0, 0) with vertices at (0, negative 4) and (0, 4). The slant asymptotes are shown but not labeled.](../../media/CNX_Precalc_Figure_10_02_215n.jpg)
...
52. ![A horizontal hyperbola centered at (1, 1) with  vertices at (1 minus square root of 2, 1) and (1 + square root of 2, 1) and foci at (1 minus square root of 5, 1) and (1 + square root of 5, 1)](../../media/CNX_Precalc_Figure_10_02_216n.jpg)

53. ![A vertical hyperbola centered at (negative 1, 0) with vertices at (negative 1, negative 3) and (negative 1, 3) and foci at (negative 1, negative 3 square root of 2) and (negative 1, 3 square root of 2).](../../media/CNX_Precalc_Figure_10_02_217n.jpg)
...
54. ![A vertical hyperbola centered at (3, 1) with vertices at (3, 1 minus square root of 2) and (3, 1 + square root of 2) and foci at (3, 1 minus square root of 7) and (3, 1 + square root of 7).](../../media/CNX_Precalc_Figure_10_02_218n.jpg)

55. ![A horizontal hyperbola centered at (negative 3, negative 3) with vertices at (negative 8, negative 3) and (2, negative 3) and foci at (negative 3 minus 5 square root of 2, negative 3) and (negative 3 + 5 square root of 2, negative 3).](../../media/CNX_Precalc_Figure_10_02_219.jpg)
...
![Image](../../media/CNX_Precalc_Figure_10_02_226.jpg)
</details>

58. $\frac{{\left(x-2\right)}^{2}}{16}-\frac{{\left(y+3\right)}^{2}}{25}=1$

59. $-4{x}^{2}-16x+{y}^{2}-2y-19=0$
...
![Image](../../media/CNX_Precalc_Figure_10_02_228.jpg)
</details>

60. $4{x}^{2}-24x-{y}^{2}-4y+16=0$
...
61. The hedge will follow the asymptotes $y=x\phantom{\rule{0.5em}{0ex}}\text{and\}y=-x,$ and its closest distance to the center fountain is 5 yards.
...
![Image](../../media/CNX_Precalc_Figure_10_02_220.jpg)
</details>

62. The hedge will follow the asymptotes $y=2x\phantom{\rule{0.5em}{0ex}}\text{and\}y=\mathrm{-2}x,$ and its closest distance to the center fountain is 6 yards.
...
![Image](../../media/CNX_Precalc_Figure_10_02_222.jpg)
</details>
...
65. The hedge will follow the asymptotes $\text{}y=\frac{3}{4}x$ and $y=-\frac{3}{4}x,$ and its closest distance to the center fountain is 20 yards.
...
![Image](../../media/CNX_Precalc_Figure_10_02_224.jpg)
</details>
...
67. The object enters along a path approximated by the line $y=2x-2$ and passes within 0.5 au of the sun at its closest approach, so the sun is one focus of the hyperbola. It then departs the solar system along a path approximated by the line $y=\mathrm{-2}x+2.$
...
68. The object enters along a path approximated by the line $y=0.5x+2$ and passes within 1 au of the sun at its closest approach, so the sun is one focus of the hyperbola. It then departs the solar system along a path approximated by the line $y=\mathrm{-0.5}x-2.$

69. The object enters along a path approximated by the line $y=\frac{1}{3}x-1$ and passes within 1 au of the sun at its closest approach, so the sun is one focus of the hyperbola. It then departs the solar system along a path approximated by the line $\text{}y=-\frac{1}{3}x+1.$
...
70. The object enters along a path approximated by the line $y=3x-9$ and passes within 1 au of the sun at its closest approach, so the sun is one focus of the hyperbola. It then departs the solar system along a path approximated by the line $y=\mathrm{-3}x+9.$
