...
Find the BMI for a man of height 5′11 who weighs 180 pounds.
Find the weight of a man of height 5′11 and who has a BMI of 22.3 .
...
Practice Makes PerfectFind the value of a function
...
For the function f(x)=3x2-2x+1 find
...
For the function g(x)=-4+3x+19 , find the following. Make sure to give exact values.
...
Note that each value in the domain is also known as an input value, or independent variable, and is often labeled with the lowercase letter 
 x. Each value in the range is also known as an output value, or dependent variable, and is often labeled lowercase letter 
 y.
A function 
 
 f is a relation that assigns a single value in the range to each value in the domain. In other words, no x-values are repeated. For our example that relates the first five natural numbers to numbers double their values, this relation is a function because each element in the domain, 
...
For example, the term “odd” corresponds to three values from the range, 
...
{2,4}.
 ...
 This violates the definition of a function, so this relation is not a function. compares relations that are functions and not functions. 

 

(a) This relationship is a function because each input is associated with a single output. Note that input 
q and 
r both give output 
...
q is associated with two different outputs.
...
We name the function f;the expression is read as “fof a.”
...
a. The value 
a must be put into the function 
h to get a result. The parentheses indicate that age is input into the function; they do not indicate multiplication.We can also give an algebraic expression as the input to a function. For example 
f(
...
March
 )=31, because March has 31 days. The notation 
 
d=f(
 m
 ) reminds us that the number of days, 
 
d (the output), is dependent on the name of the month, 
...
f(
 ...
 2005
)=300, we see that the input year is 2005. The value for the output, the number of police officers 
(
N
), is 300. Remember, 
N=f(
...
f(
 ...
 2005
)=300 tells us that in the year 2005 there were 300 police officers in the town.
...
y=y(x),
  meaning “y is a function of x?”Yes, this is often done, especially in applied subjects that use higher math, such as physics and engineering. However, in exploring math itself we like to maintain a distinction between a function such as 
 
f,
  which is a rule or procedure, and the output 
y we get by applying 
f to a particular input 
...
Representing Functions Using TablesA common method of representing functions is in the form of a table. The table rows or columns display the corresponding input and output values. In some cases, these values represent all we know about the relationship; other times, the table provides a few select examples from a more complete relationship.

  lists the input number of each month (January = 1, February = 2, and so on) and the output value of the number of days in that month. This information represents all we know about the months and days for a given year (that is not a leap year). Note that, in this table, we define a days-in-a-month function 
f where 
D=f(
m
...
g is the name of the function that takes the input 
n and gives the output 
Q. 
...
a+h, we must use the distributive property.
...
Substitute the original function h(p)=
 p
 2
...
p−1
 )=0 (or both of them equal 0). We will set each factor equal to 0 and solve for 
 p in each case.
...
This gives us two solutions. The output 
 h(
 p
 )=3 when the input is either 
 p=1 or 
 p=−3. We can also verify by graphing as in . The graph verifies that 
 h(
 1
 )=h(
...
2n+6p=12 expresses a functional relationship between 
n
and 
...
=1 represent a function with 
 x as input and 
 y as output? If so, express the relationship as a function 
 y=f(
 x
 ).
...
, if we want to express 
 y as a function of 
 x, there is no simple algebraic formula involving only 
 x that equals 
 y. However, each 
 x does determine a unique value for 
 y, and there are mathematical procedures by which 
 y can be found to any desired accuracy. In this case, we say that the equation gives an implicit (implied) rule for 
 y as a function of 
 x, even though the formula cannot be written explicitly.
...
P at the input value of “goldfish.” We would write 
...
P seems ideally suited to this function, more so than writing it in paragraph or function form.
...
When we input 2 into the function 
 g, our output is 6. When we input 4 into the function 
 g, our output is also 6.
 

Try It

 Using the table from Evaluating and Solving a Tabular Function above, evaluate 
...
ⓑ To solve 
 f(
 x
 )=4, we find the output value 
 
4
  on the vertical axis. Moving horizontally along the line 
 y=4, we locate two points of the curve with output value 
...
To visualize this concept, let’s look again at the two simple functions sketched in (a) and (b). The function in part (a) shows a relationship that is not a one-to-one function because inputs 
 q and 
 r both give output 
 n. The function in part (b) shows a relationship that is a one-to-one function because each input is associated with a single output.
 ...
 r has a unique area measure given by 
...
2
 ...
 r, there is only one output, A. The area is a function of radius 
...
The most common graphs name the input value 
 ...
 f. The graph of the function is the set of all points 
 (x,y) in the plane that satisfies the equation 
 
y=f(
 x
 ). If the function is defined for only a few input values, then the graph of the function is only a few points, where the x-coordinate of each point is an input value and the y-coordinate of each point is the corresponding output value. For example, the black dots on the graph in  tell us that 
 f(
 0
 )=2 and 
 f(
 6
 )=1. However, the set of all points 
 (x,y) satisfying 
 y=f(
 x
 ) is a curve. The curve shown includes 
 (0,2) and 
 (6,1) because the curve passes through those points.
...
Inspect the graph to see if any vertical line drawn would intersect the curve more than once.
 ...
 Which of the graphs in  represent(s) a function 
...
AlgebraicFor the following exercises, determine whether the relation represents a function.
...
NumericFor the following exercises, determine whether the relation represents a function.
...
f represented in the table below.
...
The amount of garbage, 
 G, produced by a city with population 
 p is given by 
 G=f(
 p
 ). 
...
The number of cubic yards of dirt, 
 D, needed to cover a garden with area 
 a square feet is given by 
 D=g(
 a
 ).
...
Let 
 ...
 t years after 1990. Explain the meaning of each statement:
...
Let 
 ...
 t seconds after launching. Explain the meaning of each statement:
